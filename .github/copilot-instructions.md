...
Eleventy static site generator blog with minimalist, monospace design. Source files in `src/`, built to `_site/`. Posts are Markdown with Liquid templating. Site config in `src/_data/site.json`. **ES module config** (not CommonJS) in `.eleventy.js` with async site data loading.
...
6. **Include disclaimer for cybersecurity posts**: If the post involves techniques that could be used to attack or compromise computers/systems (e.g., Wi-Fi attacks, network sniffing, privilege escalation), include the disclaimer at the top: `{% include 'disclaimer.liquid' %}`
